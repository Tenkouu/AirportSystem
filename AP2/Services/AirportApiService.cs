...
namespace AP2.Services
{
    public class AirportApiService
    {
        ...
        public AirportApiService()
        {
            ...
            _baseUrl = "http://localhost:5000/api"; // Your API base URL
        }
        ...
        public async Task<List<FlightApiResponse>> GetFlightsAsync()
        {
            try
            {
                ...
                var flights = JsonSerializer.Deserialize<List<FlightApiResponse>>(response, new JsonSerializerOptions
                {
        ...
        public async Task<bool> UpdateFlightStatusAsync(int flightId, string status)
        {
            ...
            {
                throw new Exception($"Failed to update flight status: {ex.Message}");
            }
        }
        ...
        public async Task<PassengerApiResponse> GetPassengerByPassportAsync(string passportNumber)
        {
            try
            {
                ...
                var passengers = JsonSerializer.Deserialize<List<PassengerApiResponse>>(response, new JsonSerializerOptions
                {
                    PropertyNameCaseInsensitive = true
                });
        ...
        public async Task<CheckInApiResponse> CheckInPassengerAsync(string passportNumber, string? selectedSeatNumber = null)
        {
            try
            {
                ...
                {
                    return JsonSerializer.Deserialize<CheckInApiResponse>(responseContent, new JsonSerializerOptions
                    {
                        PropertyNameCaseInsensitive = true
                    });
                }
        ...
        public async Task<List<SeatApiResponse>> GetSeatsByFlightAsync(int flightId)
        {
            try
            {
                ...
                var seats = JsonSerializer.Deserialize<List<SeatApiResponse>>(response, new JsonSerializerOptions
                {
                    PropertyNameCaseInsensitive = true
                });
            ...
            {
                throw new Exception($"Failed to fetch seats: {ex.Message}");
            }
        }
    ...
    public class FlightApiResponse
    {
        ...
        public int FlightStatus { get; set; } // Integer enum value from API
